...
class Avro_push_cli_implemenetation_Test {
  ...
  void initialize() throws URISyntaxException {
    // the target directory has only one file
    expectedFile = Paths.get(getClass().getClassLoader().getResource(DIRECTORY + "/test_schema_too.json").toURI());
  ...
  void executing_the_cli_implementation_should_invoke_the_schmea_pusher_implementation(
    ...
    sut.run();
    // then the mock pusher should be pushed with the expected arguments

...
public class SchemaProducer<K, V> implements Producer<K, V> {
  ...
  public SchemaProducer(
      Map<String, Object> configs, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
    this(
        new ProducerConfig(appendSerializerToConfig(configs, keySerializer, valueSerializer)),
        keySerializer,
        valueSerializer);
  }
  ...
  SchemaProducer(
      ProducerConfig config, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
    ...
    if (transactionalId == null)
      ...
      logContext =
          new LogContext(
              String.format(
                  "[Producer clientId=%s, transactionalId=%s] ", clientId, transactionalId));
  ...
  static Map<String, Object> appendSerializerToConfig(
      Map<String, Object> configs, Serializer<?> keySerializer, Serializer<?> valueSerializer) {
    // validate serializer configuration, if the passed serializer instance is null, the user must
    // explicitly set a valid serializer configuration value
    ...
    if (keySerializer != null)
      ...
      throw new ConfigException(
          ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, null, "must be non-null.");
    if (valueSerializer != null)
      ...
      throw new ConfigException(
          ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, null, "must be non-null.");
    return newConfigs;
  }
